var LANGUAGE = {
    // *****// 头部
    '登录': {
        'ZH': '登录',
        'EN': 'Login'
    },
    '注册': {
        'ZH': '注册',
        'EN': 'Register'
    },
    '首页': {
        'ZH': '首页',
        'EN': 'Home'
    },
    '交易': {
        'ZH': '交易',
        'EN': 'Trading'
    },
    'Trades': {
        'ZH': '交易',
        'EN': 'Trades'
    },
    '场外交易': {
        'ZH': '场外交易',
        'EN': 'Fiat'
    },
    '币币交易': {
        'ZH': '币币交易',
        'EN': 'Exchange'
    },
    '邀请好友': {
        'ZH': '邀请好友',
        'EN': 'Invitiation'
    },
    '资产': {
        'ZH': '资产',
        'EN': 'Balances'
    },
    '总资产': {
        'ZH': '总资产',
        'EN': 'Total value'
    },
    '可用资产': {
        'ZH': '可用资产',
        'EN': 'Available'
    },
    '冻结资产': {
        'ZH': '冻结资产',
        'EN': 'Frozen'
    },
    '去购买': {
        'ZH': '去购买',
        'EN': 'Buy'
    },
    '去出售': {
        'ZH': '去出售',
        'EN': 'Sell'
    },
    '订单': {
        'ZH': '订单',
        'EN': 'Orders'
    },
    '查看更多': {
        'ZH': '查看更多',
        'EN': 'More'
    },
    '场外交易订单': {
        'ZH': '场外交易订单',
        'EN': 'Fiat orders'
    },
    '币币交易订单': {
        'ZH': '币币交易订单',
        'EN': 'Exchange orders'
    },
    '用户中心': {
        'ZH': '用户中心',
        'EN': 'User center'
    },
    '退出登录': {
        'ZH': '退出登录',
        'EN': 'Log out '
    },
    // *****/ 尾部
    '客服': {
        'ZH': '客服',
        'EN': 'Customer service '
    },
    'qq': {
        'ZH': 'QQ',
        'EN': 'QQ'
    },
    'wx': {
        'ZH': '微信',
        'EN': 'WeChat'
    },
    '工具': {
        'ZH': '工具',
        'EN': 'Tools'
    },
    '帮助中心': {
        'ZH': '帮助中心',
        'EN': 'Support'
    },
    '关于我们': {
        'ZH': '关于我们',
        'EN': 'About us'
    },
    '平台介绍': {
        'ZH': '平台介绍',
        'EN': '平台介绍'
    },
    '联系我们': {
        'ZH': '联系我们',
        'EN': 'Contact us'
    },
    '公告': {
        'ZH': '公告',
        'EN': 'Notice'
    },
    '条款说明': {
        'ZH': '条款说明',
        'EN': 'Terms of Service'
    },
    '用户协议': {
        'ZH': '用户协议',
        'EN': 'User Agreement'
    },
    '隐私条款': {
        'ZH': '隐私条款',
        'EN': 'Privacy Policy'
    },
    '法律声明': {
        'ZH': '法律声明',
        'EN': 'Legal Statement'
    },
    '费率说明': {
        'ZH': '费率说明',
        'EN': 'Fee Rate'
    },

    // *****///// 首页
    '区块链技术应用实验平台': {
        'ZH': '区块链技术应用实验平台',
        'EN': 'blockchain technology application experimental platform'
    },
    '尽情游戏': {
        'ZH': '尽情游戏',
        'EN': 'The game '
    },
    '无尽精彩': {
        'ZH': '无尽精彩',
        'EN': ' is endless'
    },
    '尽情游戏 无尽精彩': {
        'ZH': '尽情游戏 无尽精彩',
        'EN': 'The game is endless'
    },
    '驯兽师': {
        'ZH': '驯兽师',
        'EN': 'Crowned Up'
    },
    '精彩画面': {
        'ZH': '精彩画面',
        'EN': 'Wonderful view '
    },
    '任你游': {
        'ZH': '任你游',
        'EN': 'full of passion'
    },
    '精彩画面, 任你游': {
        'ZH': '精彩画面, 任你游',
        'EN': 'Wonderful view full of passion'
    },
    '进入游戏': {
        'ZH': '进入游戏',
        'EN': 'Enter'
    },
    '二手车兑换': {
        'ZH': '二手车兑换',
        'EN': 'Used-car Trade'
    },
    '开始兑换': {
        'ZH': '开始兑换',
        'EN': 'Used-car Trade'
    },
    '即将推出': {
        'ZH': '即将推出',
        'EN': 'Coming soon'
    },
    '即将推出，敬请期待': {
        'ZH': '即将推出，敬请期待',
        'EN': 'Coming soon'
    },
    '敬请期待': {
        'ZH': '敬请期待',
        'EN': 'Coming soon'
    },
    '全球实时行情': {
        'ZH': '全球实时行情',
        'EN': 'Global real-time market'
    },
    '双向交易': {
        'ZH': '双向交易',
        'EN': 'two-way trading'
    },
    '买卖自由': {
        'ZH': '买卖自由',
        'EN': 'Safe'
    },
    '安全可靠': {
        'ZH': '安全可靠',
        'EN': 'Teliable'
    },
    '为什么要选择？': {
        'ZH': '为什么要选择？',
        'EN': 'WHY CHOOSE?'
    },
    '支持多种数字货币的币币交易与场外交易': {
        'ZH': '支持多种数字货币的币币交易与场外交易',
        'EN': 'Transactions of fiat and exchange which support multiple digital currency'
    },
    '快速方便': {
        'ZH': '快速方便',
        'EN': 'quick and easy'
    },
    '冷存储': {
        'ZH': '冷存储',
        'EN': ''
    },
    '多重加密等银行级别安全技术': {
        'ZH': '多重加密等银行级别安全技术',
        'EN': 'Multiple encryption and other bank-level security technologies'
    },
    '十年技术安全团队': {
        'ZH': '十年技术安全团队',
        'EN': "10 years' technical security team"
    },
    '多重保障资产安全': {
        'ZH': '多重保障资产安全',
        'EN': 'Multiple safeguards of fund security'
    },
    '交易便捷': {
        'ZH': '交易便捷',
        'EN': 'Convenient'
    },
    '支持多终端交易': {
        'ZH': '支持多终端交易',
        'EN': 'Support multi-terminal transactions'
    },
    '行情及时掌握': {
        'ZH': '行情及时掌握',
        'EN': 'grasp market timely'
    },
    '快速交易': {
        'ZH': '快速交易',
        'EN': 'transact fast'
    },
    '贴心服务': {
        'ZH': '贴心服务',
        'EN': 'caring service'
    },
    '全球市场': {
        'ZH': '全球市场',
        'EN': 'global market'
    },
    '交易随时随地': {
        'ZH': '交易随时随地',
        'EN': 'Trading anywhere, anytime'
    },

    /////////////////////////////////////////////////// 商城
    '商城': {
        'ZH': '商城',
        'EN': 'Mall'
    },
    '区块链游戏': {
        'ZH': '区块链游戏',
        'EN': 'Blockchain game'
    },
    '游戏余额': {
        'ZH': '游戏余额',
        'EN': 'Balances'
    },
    '转入游戏': {
        'ZH': '转入游戏',
        'EN': 'Deposit'
    },
    '充值': {
        'ZH': '充值',
        'EN': 'Deposit'
    },
    '请输入充值数量': {
        'ZH': '请输入充值数量',
        'EN': 'Please enter the deposit amount'
    },
    '充值数量不能为空': {
        'ZH': '充值数量不能为空',
        'EN': 'The amount of deposit can not be empty'
    },

    /////////////////////////////////////////////////// 币币交易
    '邀请好友注册': {
        'ZH': '邀请好友注册',
        'EN': 'Invite friends to register'
    },
    '轻松获得交易返佣': {
        'ZH': '轻松获得交易返佣',
        'EN': 'easily get cashback'
    },
    '立即邀请': {
        'ZH': '立即邀请',
        'EN': 'invite immediately'
    },
    '净资产折合': {
        'ZH': '净资产折合',
        'EN': 'Estimated value'
    },
    '或': {
        'ZH': '或',
        'EN': 'or'
    },
    '开始交易': {
        'ZH': '开始交易',
        'EN': 'Start trading'
    },
    '币种': {
        'ZH': '币种',
        'EN': 'Currency'
    },
    '最新价': {
        'ZH': '最新价',
        'EN': 'Latest price'
    },
    '涨幅': {
        'ZH': '涨幅',
        'EN': 'Change'
    },
    '主区': {
        'ZH': '主区',
        'EN': ' main '
    },
    '高': {
        'ZH': '高',
        'EN': 'High'
    },
    '低': {
        'ZH': '低',
        'EN': 'Low'
    },
    '24H量': {
        'ZH': '24H量',
        'EN': '24H Volume'
    },
    '限价交易': {
        'ZH': '限价交易',
        'EN': 'Limit'
    },
    '市价交易': {
        'ZH': '市价交易',
        'EN': 'Market'
    },
    '费率': {
        'ZH': '费率',
        'EN': 'Fee rate'
    },
    '可用': {
        'ZH': '可用',
        'EN': 'Available'
    },
    '充币': {
        'ZH': '充币',
        'EN': 'Deposit'
    },
    '买入价': {
        'ZH': '买入价',
        'EN': 'Price'
    },
    '买入量': {
        'ZH': '买入量',
        'EN': 'Amount'
    },
    '买入': {
        'ZH': '买入',
        'EN': 'Buy'
    },
    '卖出价': {
        'ZH': '卖出价',
        'EN': 'Price'
    },
    '卖出量': {
        'ZH': '卖出量',
        'EN': 'Amount'
    },
    '卖出': {
        'ZH': '卖出',
        'EN': 'Sell'
    },
    '最低价': {
        'ZH': '最低价',
        'EN': 'Lowest price'
    },
    '数量': {
        'ZH': '数量',
        'EN': 'Amount'
    },
    'count': {
        'ZH': '数量',
        'EN': 'Count'
    },
    '剩余数量': {
        'ZH': '剩余数量',
        'EN': 'Available'
    },
    '当前委托': {
        'ZH': '当前委托',
        'EN': 'Open orders'
    },
    '交易额': {
        'ZH': '交易额',
        'EN': 'Total'
    },
    '历史委托': {
        'ZH': '历史委托',
        'EN': 'Order history'
    },
    '全部': {
        'ZH': '全部',
        'EN': 'All'
    },
    '时间': {
        'ZH': '时间',
        'EN': 'Date'
    },
    '交易对': {
        'ZH': '交易对',
        'EN': 'Direction'
    },
    '方向': {
        'ZH': '方向',
        'EN': 'Type'
    },
    'Amt': {
        'ZH': '数量',
        'EN': 'Amt'
    },
    '委托总额': {
        'ZH': '委托总额',
        'EN': 'Total'
    },
    '已成交': {
        'ZH': '已成交',
        'EN': 'Executed'
    },
    '未成交': {
        'ZH': '未成交',
        'EN': 'Unexecuted'
    },
    '操作': {
        'ZH': '操作',
        'EN': 'Action'
    },
    '暂无记录': {
        'ZH': '暂无记录',
        'EN': `No open orders yet 
        Go to the Trade tab and make your order!`
    },
    '委托量': {
        'ZH': '委托量',
        'EN': 'Amount'
    },
    '成交均价': {
        'ZH': '成交均价',
        'EN': 'Avg price'
    },
    '状态': {
        'ZH': '状态',
        'EN': 'Status'
    },
    '所有记录': {
        'ZH': '所有记录',
        'EN': 'All orders'
    },
    '深度图': {
        'ZH': '深度图',
        'EN': 'Depth chart'
    },
    '实时成交': {
        'ZH': '实时成交',
        'EN': 'Market trades'
    },
    '币种资料': {
        'ZH': '币种资料',
        'EN': 'Basic Information'
    },
    '简介': {
        'ZH': '简介',
        'EN': 'Introduction'
    },
    '发行时间': {
        'ZH': '发行时间',
        'EN': 'Issue Date'
    },
    '发行总量': {
        'ZH': '发行总量',
        'EN': 'Total Amount'
    },
    '流通总量': {
        'ZH': '流通总量',
        'EN': 'Circulation'
    },
    '白皮书': {
        'ZH': '白皮书',
        'EN': 'White paper'
    },
    '官网': {
        'ZH': '官网',
        'EN': 'Official website'
    },
    '区块查询': {
        'ZH': '区块查询',
        'EN': 'Block explorer'
    },
    '单位跳动基数为': {
        'ZH': '单位跳动基数为',
        'EN': 'interval'
    },

    // ***** 场外交易-购买
    '全部货币': {
        'ZH': '全部货币',
        'EN': 'All currencies'
    },
    '全部付款方式': {
        'ZH': '全部付款方式',
        'EN': 'All payment methods'
    },
    '搜广告': {
        'ZH': '搜广告',
        'EN': 'Search advertisement'
    },
    '搜用户': {
        'ZH': '搜用户',
        'EN': 'Search user'
    },
    '货币': {
        'ZH': '货币',
        'EN': 'Currency'
    },
    '请选择': {
        'ZH': '请选择',
        'EN': 'Please choose'
    },
    'CNY人民币': {
        'ZH': 'CNY人民币',
        'EN': 'CNY RMB'
    },
    'USD美元': {
        'ZH': 'USD美元',
        'EN': 'USD dollar'
    },
    '全部货币，全部付款方式': {
        'ZH': '全部货币，全部付款方式',
        'EN': 'Select'
    },
    '支付宝': {
        'ZH': '支付宝',
        'EN': 'Alipay'
    },
    '微信': {
        'ZH': '微信',
        'EN': 'Wechat'
    },
    '银行卡转账': {
        'ZH': '银行卡转账',
        'EN': 'Bankcard'
    },
    '请输入完整用户名': {
        'ZH': '请输入完整用户名',
        'EN': 'Please enter a full username'
    },
    '搜索': {
        'ZH': '搜索',
        'EN': 'Search'
    },
    '昵称': {
        'ZH': '昵称',
        'EN': 'Merchant'
    },
    '付款方式': {
        'ZH': '付款方式',
        'EN': 'Payment Method'
    },
    '限额': {
        'ZH': '限额',
        'EN': 'Limits'
    },
    'Avaliable': {
        'ZH': '数量',
        'EN': 'Avaliable'
    },

    /////////////////////////////////////////////////// 场外交易-订单详情
    '订单详情': {
        'ZH': '订单详情',
        'EN': 'Order details'
    },
    '订单信息': {
        'ZH': '订单信息',
        'EN': 'Order information'
    },
    '交易价格': {
        'ZH': '交易价格',
        'EN': 'Price'
    },
    '交易数量': {
        'ZH': '交易数量',
        'EN': 'Quantity'
    },
    'Quantity': {
        'ZH': '数量',
        'EN': 'Quantity'
    },
    '交易金额': {
        'ZH': '交易金额',
        'EN': 'Amount'
    },
    '报价': {
        'ZH': '报价',
        'EN': 'Quotation'
    },
    '交易限额': {
        'ZH': '交易限额',
        'EN': 'Limit'
    },
    '聊天': {
        'ZH': '聊天',
        'EN': 'Chat'
    },
    '按回车发送消息': {
        'ZH': '按回车发送消息',
        'EN': 'Press enter to send the message'
    },
    '电子邮箱': {
        'ZH': '电子邮箱',
        'EN': 'Email address'
    },
    '手机号码': {
        'ZH': '手机号码',
        'EN': 'Cellphone number'
    },
    '实名认证': {
        'ZH': '实名认证',
        'EN': 'Certification'
    },
    '用户创建时间': {
        'ZH': '用户创建时间',
        'EN': 'User creation date'
    },
    '交易操作': {
        'ZH': '交易操作',
        'EN': 'Action'
    },
    '订单编号': {
        'ZH': '订单编号',
        'EN': 'No.'
    },
    '广告留言': {
        'ZH': '广告留言',
        'EN': 'Message'
    },
    '标记付款': {
        'ZH': '标记付款',
        'EN': 'Mark payment'
    },
    '取消交易': {
        'ZH': '取消交易',
        'EN': 'Cancel'
    },
    '交易评价': {
        'ZH': '交易评价',
        'EN': 'Comment'
    },
    '解冻货币': {
        'ZH': '解冻货币',
        'EN': 'Unfrozen'
    },
    '申请仲裁': {
        'ZH': '申请仲裁',
        'EN': 'Apply for arbitration'
    },
    '发送图片': {
        'ZH': '发送图片',
        'EN': 'Send picture'
    },
    '选择': {
        'ZH': '选择',
        'EN': 'Select'
    },
    '选择图片': {
        'ZH': '选择图片',
        'EN': 'Select image'
    },
    '关闭': {
        'ZH': '关闭',
        'EN': 'Close'
    },
    '发送': {
        'ZH': '发送',
        'EN': 'Send'
    },
    '预览': {
        'ZH': '预览',
        'EN': 'Preview'
    },
    '进度': {
        'ZH': '进度',
        'EN': 'Progress'
    },
    '查看图片': {
        'ZH': '查看图片',
        'EN': 'Review image'
    },
    '请填写您申请仲裁的理由': {
        'ZH': '请填写您申请仲裁的理由',
        'EN': 'Please fill in your reasons for applying for arbitration'
    },
    '放弃': {
        'ZH': '放弃',
        'EN': 'Give up'
    },
    '确认申请': {
        'ZH': '确认申请',
        'EN': 'Confirm'
    },
    '交易有何印象': {
        'ZH': '交易有何印象',
        'EN': 'What is your comment on this transaction'
    },
    '快來评价吧': {
        'ZH': '快來评价吧',
        'EN': 'Come and make a comment'
    },
    '交易有何印象？快來评价吧': {
        'ZH': '交易有何印象？快來评价吧',
        'EN': 'What is your comment on this transaction? Come and make a comment'
    },
    '好评': {
        'ZH': '好评',
        'EN': 'Praises'
    },
    '差评': {
        'ZH': '差评',
        'EN': 'Complaints'
    },
    '提交': {
        'ZH': '提交',
        'EN': 'Submit'
    },

    // *****/ 场外交易-广告管理
    '广告管理': {
        'ZH': '广告管理',
        'EN': 'Ads'
    },
    '购买广告': {
        'ZH': '购买广告',
        'EN': 'Buy Ad'
    },
    '出售广告': {
        'ZH': '出售广告',
        'EN': 'Sell Ad'
    },
    '编号': {
        'ZH': '编号',
        'EN': 'No.'
    },
    '广告类型': {
        'ZH': '广告类型',
        'EN': 'Type'
    },
    '溢价比例': {
        'ZH': '溢价比例',
        'EN': 'Premium rate'
    },
    '创建时间': {
        'ZH': '创建时间',
        'EN': 'Time'
    },
    '交易状态': {
        'ZH': '交易状态',
        'EN': 'Status'
    },
    '暂无广告': {
        'ZH': '暂无广告',
        'EN': 'No advertisement'
    },

    ///////////////////////////////////////////////// 场外交易-信任管理
    '信任管理': {
        'ZH': '信任管理',
        'EN': 'Trust'
    },
    '我信任的': {
        'ZH': '我信任的',
        'EN': 'Trust'
    },
    '信任我的': {
        'ZH': '信任我的',
        'EN': 'Trusted'
    },
    '我屏蔽的': {
        'ZH': '我屏蔽的',
        'EN': 'Blacklist'
    },

    // *****/ 发布广告
    '发布广告': {
        'ZH': '发布广告',
        'EN': 'Create Ad'
    },
    '发布': {
        'ZH': '发布',
        'EN': 'Release '
    },
    '交易广告': {
        'ZH': '交易广告',
        'EN': ' trading advertisement'
    },
    '您想要创建什么样的交易广告': {
        'ZH': '您想要创建什么样的交易广告',
        'EN': 'What kind of trading advertisement do you want to create?'
    },
    '如果您希望出售': {
        'ZH': '如果您希望出售',
        'EN': 'If you wish to sell'
    },
    '请确保您在本网站的钱包中可用余额充足': {
        'ZH': '请确保您在本网站的钱包中可用余额充足',
        'EN': 'Please ensure enough amount.'
    },
    '交易类型': {
        'ZH': '交易类型',
        'EN': 'Type'
    },
    '选择广告类型': {
        'ZH': '选择广告类型',
        'EN': 'Select advertisement type'
    },
    '选择广告类型说明': {
        'ZH': '您想要创建什么样的交易广告？如果您希望出售，请确保您在本网站的钱包中可用余额充足。',
        'EN': 'What kind of trading advertisement do you want to create? If you wish to sell, Please ensure enough available balances'
    },
    '在线出售': {
        'ZH': '在线出售',
        'EN': 'On-line sell'
    },
    '在线购买': {
        'ZH': '在线购买',
        'EN': 'On-line buy'
    },
    '更多信息': {
        'ZH': '更多信息',
        'EN': 'More information'
    },
    '您希望交易付款的货币类型': {
        'ZH': '您希望交易付款的货币类型',
        'EN': 'The currency type you would like to make trade payment'
    },
    '溢价': {
        'ZH': '溢价',
        'EN': 'Premium'
    },
    '价格': {
        'ZH': '价格',
        'EN': 'Price'
    },
    '选填': {
        'ZH': '选填',
        'EN': 'Optional'
    },
    '广告最高可成交的价格': {
        'ZH': '广告最高可成交的价格',
        'EN': 'The highest advertised price'
    },
    '最小限额': {
        'ZH': '最小限额',
        'EN': 'Minumum limit'
    },
    '请输入最小限额': {
        'ZH': '请输入最小限额',
        'EN': 'Please enter the minimum limit'
    },
    '最大限额': {
        'ZH': '最大限额',
        'EN': 'Maximum limit'
    },
    '请输入最大限额': {
        'ZH': '请输入最大限额',
        'EN': 'please enter the maximum limit'
    },
    '购买总量': {
        'ZH': '购买总量',
        'EN': 'Total amount'
    },
    '账户可用余额': {
        'ZH': '账户可用余额',
        'EN': 'Available amount'
    },
    '请输入购买币的总量': {
        'ZH': '请输入购买币的总量',
        'EN': 'Please enter the total amount'
    },
    '收款方式': {
        'ZH': '收款方式',
        'EN': 'Payment Methods'
    },
    '请输入有关您交易的相关条款或备注您的支付方式': {
        'ZH': '请输入有关您交易的相关条款或备注您的支付方式',
        'EN': 'Please enter the terms of your transaction or remarks about your payment method'
    },
    '付款备注里不得出现 BTC/ETH、数字货币、区块链等字眼。': {
        'ZH': '付款备注里不得出现 BTC/ETH、数字货币、区块链等字眼。',
        'EN': 'Please enter the terms of your transaction or remarks about your payment method'
    },
    '谢谢': {
        'ZH': '谢谢',
        'EN': 'thank you'
    },
    '显示高级设置': {
        'ZH': '显示高级设置',
        'EN': 'Display advanced Settings'
    },
    '保存草稿': {
        'ZH': '保存草稿',
        'EN': 'Save Draft'
    },
    '仅限受信任的交易者': {
        'ZH': '仅限受信任的交易者',
        'EN': 'Only trusted traders'
    },
    '启用': {
        'ZH': '启用',
        'EN': 'On'
    },
    '开放时间': {
        'ZH': '开放时间',
        'EN': 'Opening hours'
    },
    '任何时候': {
        'ZH': '任何时候',
        'EN': 'Anytime'
    },
    '自定义': {
        'ZH': '自定义',
        'EN': 'User-defined'
    },
    '星期一': {
        'ZH': '星期一',
        'EN': 'Monday'
    },
    '星期二': {
        'ZH': '星期二',
        'EN': 'Tuesday'
    },
    '星期三': {
        'ZH': '星期三',
        'EN': 'Wednesday'
    },
    '星期四': {
        'ZH': '星期四',
        'EN': 'Thursday'
    },
    '星期五': {
        'ZH': '星期五',
        'EN': 'Friday'
    },
    '星期六': {
        'ZH': '星期六',
        'EN': 'Saturday'
    },
    '星期日': {
        'ZH': '星期日',
        'EN': 'Sunday'
    },
    '立即发布': {
        'ZH': '立即发布',
        'EN': 'Release immediately'
    },
    '下架': {
        'ZH': '下架',
        'EN': 'Off'
    },

    // ***** 购买详情
    '购买详情': {
        'ZH': '购买详情',
        'EN': 'Purchase details'
    },
    '已认证': {
        'ZH': '已认证',
        'EN': 'Certified'
    },
    '交易次数': {
        'ZH': '交易次数',
        'EN': 'Trades'
    },
    '信任人数': {
        'ZH': '信任人数',
        'EN': 'Trusted'
    },
    '好评率': {
        'ZH': '好评率',
        'EN': 'Feedback'
    },
    '历史成交': {
        'ZH': '历史成交',
        'EN': 'Trades history'
    },
    '联系对方': {
        'ZH': '联系对方',
        'EN': 'Message'
    },
    '分钟': {
        'ZH': '分钟',
        'EN': 'Minute'
    },
    '付款期限': {
        'ZH': '付款期限',
        'EN': 'The payment term'
    },
    '卖家留言': {
        'ZH': '卖家留言',
        'EN': 'Message from seller'
    },
    '买家留言': {
        'ZH': '卖家留言',
        'EN': 'Message from buyer'
    },
    '你想买卖多少': {
        'ZH': '你想买卖多少',
        'EN': 'How much do you want to buy/sell'
    },
    '广告剩余可交易量': {
        'ZH': '广告剩余可交易量',
        'EN': 'advertisement left to be traded'
    },
    '请输入您购买的金额': {
        'ZH': '请输入您购买的金额',
        'EN': 'Please enter the purchase amount'
    },
    '请输入您购买的数量': {
        'ZH': '请输入您购买的数量',
        'EN': 'Please enter the quantity you purchased'
    },
    '请输入您出售的金额': {
        'ZH': '请输入您出售的金额',
        'EN': 'Please enter the amount you sell'
    },
    '请输入您出售的数量': {
        'ZH': '请输入您出售的数量',
        'EN': 'Please enter the quantity you sell'
    },
    '立即购买': {
        'ZH': '立即购买',
        'EN': 'Buy now'
    },
    '交易提醒': {
        'ZH': '交易提醒',
        'EN': 'Trading reminder'
    },
    '交易密码': {
        'ZH': '交易密码',
        'EN': 'Trading password'
    },
    '确认': {
        'ZH': '确认',
        'EN': 'Confirm'
    },
    '取消': {
        'ZH': '取消',
        'EN': 'Cancel'
    },
    '下单确认': {
        'ZH': '下单确认',
        'EN': 'Order confirmation'
    },
    '购买价格': {
        'ZH': '购买价格',
        'EN': 'Purchase price'
    },
    '购买金额': {
        'ZH': '购买金额',
        'EN': 'Purchase amount'
    },
    '购买数量': {
        'ZH': '购买数量',
        'EN': 'Purchase quantity'
    },
    '出售价格': {
        'ZH': '出售价格',
        'EN': 'Sale price'
    },
    '出售金额': {
        'ZH': '出售金额',
        'EN': 'Sale amount'
    },
    '出售数量': {
        'ZH': '出售数量',
        'EN': 'Sale quantity'
    },
    '提醒': {
        'ZH': '提醒',
        'EN': 'Remind'
    },
    '请确认价格再下单': {
        'ZH': '请确认价格再下单',
        'EN': 'Please confirm the price and place the order'
    },
    '下单彼此交易的': {
        'ZH': '下单彼此交易的',
        'EN': 'Orders traded will be managed and locked'
    },
    '请放心购买': {
        'ZH': '请放心购买',
        'EN': 'Please feel free to buy.'
    },
    '提醒：请确认价格再下单,下单彼此交易的': {
        'ZH': '提醒：请确认价格再下单,下单彼此交易的',
        'EN': 'Reminder: please confirm that the price is placed on the order. The '
    },
    '将被托管锁定，请放心购买。': {
        'ZH': '将被托管锁定，请放心购买。',
        'EN': ' will be placed under the custody order. Please rest assured.'
    },
    '确认购买': {
        'ZH': '确认购买',
        'EN': 'Confirm'
    },
    '放弃购买': {
        'ZH': '放弃购买',
        'EN': 'Give up'
    },
    '确认出售': {
        'ZH': '确认出售',
        'EN': 'Confirm'
    },
    '放弃出售': {
        'ZH': '放弃出售',
        'EN': 'Give up'
    },
    '未认证': {
        'ZH': '未认证',
        'EN': 'Uncertified'
    },
    '你想出售多少': {
        'ZH': '你想出售多少',
        'EN': 'How much do you want to sell'
    },
    '立即出售': {
        'ZH': '立即出售',
        'EN': 'Sell immediately'
    },

    ///////////////////////////////////////// 用户详情
    '好评度': {
        'ZH': '好评度',
        'EN': 'Praises'
    },
    '历史交易': {
        'ZH': '历史交易',
        'EN': 'Trades history'
    },
    '发布的广告': {
        'ZH': '发布的广告',
        'EN': 'Releasing advertisement'
    },
    '交易币种': {
        'ZH': '交易币种',
        'EN': 'Trading currencies'
    },

    /////////////////////////////////////////// 邀请好友
    '交易总额': {
        'ZH': '交易总额',
        'EN': 'Total amount'
    },
    '成功邀请': {
        'ZH': '成功邀请',
        'EN': 'Invited'
    },
    '人': {
        'ZH': '人',
        'EN': 'people'
    },
    '已得收益': {
        'ZH': '已得收益',
        'EN': 'Proceeds'
    },
    '注册分佣奖励': {
        'ZH': '注册分佣奖励',
        'EN': 'Registration Proceeds'
    },
    '第一次币币交易分佣奖励': {
        'ZH': '第一次币币交易分佣奖励',
        'EN': 'First Currency Exchange Proceeds'
    },
    '二维码推荐': {
        'ZH': '二维码推荐',
        'EN': 'Share QR Code'
    },
    '文字推荐': {
        'ZH': '文字推荐',
        'EN': 'Share Link'
    },
    '邀请记录': {
        'ZH': '邀请记录',
        'EN': 'Invitation History'
    },
    '用户名': {
        'ZH': '用户名',
        'EN': 'Username'
    },
    '注册时间': {
        'ZH': '注册时间',
        'EN': 'Registration Date'
    },
    '佣金计算': {
        'ZH': '佣金计算',
        'EN': 'Cashback'
    },
    '注册佣金': {
        'ZH': '注册佣金',
        'EN': 'Registration'
    },
    '交易佣金': {
        'ZH': '交易佣金',
        'EN': 'Trading'
    },
    '活动规则': {
        'ZH': '活动规则',
        'EN': 'Program Rules'
    },
    '扫描二维码邀请注册': {
        'ZH': '扫描二维码邀请注册',
        'EN': 'Scan QR code to invite'
    },
    '复制下面这段文字给好友': {
        'ZH': '复制下面这段文字给好友',
        'EN': 'copy and link to share'
    },
    '好友通过你的专属链接注册成功': {
        'ZH': '好友通过你的专属链接注册成功',
        'EN': 'Friends registered through your exclusive link successfully'
    },
    '你就可以享受好友的交易提成了': {
        'ZH': '你就可以享受好友的交易提成了',
        'EN': 'You can then get cashback from your friends.'
    },
    '复制下面这段文字...': {
        'ZH': '复制下面这段文字给好友，好友通过你的专属链接注册成功，你就可以享受好友的交易提成了',
        'EN': 'Copy and link to share, Friends registered through your exclusive link successfully, You can then get cashback from your friends.'
    },

    /////////////////////////////////////////////////// 订单管理
    '进行中': {
        'ZH': '进行中',
        'EN': 'Processing'
    },
    '已结束': {
        'ZH': '已结束',
        'EN': 'Completed'
    },
    '交易伙伴': {
        'ZH': '交易伙伴',
        'EN': 'Merchant'
    },
    '类型': {
        'ZH': '类型',
        'EN': 'Type'
    },
    '暂无订单': {
        'ZH': '暂无订单',
        'EN': 'No orders'
    },

    // ***** 钱包
    '钱包-区块链技术应用实验平台': {
        'ZH': '钱包-区块链技术应用实验平台',
        'EN': 'Wallet- blockchain technology application experimental platform'
    },
    '钱包': {
        'ZH': '钱包',
        'EN': 'Wallet'
    },
    '总资产折合': {
        'ZH': '总资产折合',
        'EN': 'Total estimated value'
    },
    '币币账户与场外账户均可支持充币提币': {
        'ZH': '币币账户与场外账户均可支持充币提币',
        'EN': 'Both fiat and exchange support deposit and withdrawal'
    },
    '冻结': {
        'ZH': '冻结',
        'EN': 'Frozen'
    },
    '暂无明细': {
        'ZH': '暂无明细',
        'EN': 'No details'
    },
    '地址': {
        'ZH': '地址',
        'EN': 'Address'
    },
    '添加新地址': {
        'ZH': '添加新地址',
        'EN': 'Add the address'
    },
    '确认地址': {
        'ZH': '确认地址',
        'EN': 'Confirm the address'
    },

    '订单记录': {
        'ZH': '订单记录',
        'EN': 'Records'
    },
    '币数量': {
        'ZH': '币数量',
        'EN': 'the number of coins'
    },
    '订单号': {
        'ZH': '订单号',
        'EN': 'No.'
    },
    '订单总额': {
        'ZH': '订单总额',
        'EN': 'Order amount'
    },
    '手续费': {
        'ZH': '手续费',
        'EN': 'Fee rate'
    },
    '手续费率': {
        'ZH': '手续费率',
        'EN': 'Charges rates'
    },
    '下单时间': {
        'ZH': '下单时间',
        'EN': 'Order time'
    },
    '收款信息': {
        'ZH': '收款信息',
        'EN': 'Collection information'
    },
    '收款人': {
        'ZH': '收款人',
        'EN': 'Payee'
    },
    '账号': {
        'ZH': '账号',
        'EN': 'Account'
    },
    '开户行': {
        'ZH': '开户行',
        'EN': 'Bank of deposit'
    },
    '我已完成付款': {
        'ZH': '我已完成付款',
        'EN': 'I have finished paying'
    },
    '总金额': {
        'ZH': '总金额',
        'EN': 'Total'
    },
    '下单日期': {
        'ZH': '下单日期',
        'EN': 'Date'
    },
    '场外交易明细': {
        'ZH': '场外交易明细',
        'EN': 'Fiat transaction details'
    },
    '交易明细': {
        'ZH': '交易明细',
        'EN': 'Details'
    },
    '提现': {
        'ZH': '提现',
        'EN': 'Withdraw'
    },
    '交易买入': {
        'ZH': '交易买入',
        'EN': 'Buy'
    },
    '交易卖出': {
        'ZH': '交易卖出',
        'EN': 'Sell'
    },
    '场外承兑商购买': {
        'ZH': '场外承兑商购买',
        'EN': 'Assurer to buy'
    },
    '场外承兑商出售': {
        'ZH': '场外承兑商出售',
        'EN': 'Assurer to sell'
    },
    '交易手续费': {
        'ZH': '交易手续费',
        'EN': 'Transaction charges'
    },
    '提现手续费': {
        'ZH': '提现手续费',
        'EN': 'Withdrawal charges'
    },
    '手续费': {
        'ZH': '手续费',
        'EN': 'Fee'
    },
    '游戏充值': {
        'ZH': '转入游戏',
        'EN': 'Charge for game'
    },
    '游戏提现': {
        'ZH': '游戏提现',
        'EN': 'Withdraw from game'
    },
    '冻结解冻': {
        'ZH': '冻结解冻',
        'EN': 'Release/Unrelease'
    },
    '邀请好友收入': {
        'ZH': '邀请好友收入',
        'EN': 'Aashback from invitiation'
    },
    '冻结记录': {
        'ZH': '冻结记录',
        'EN': 'Freezing records'
    },
    '说明': {
        'ZH': '说明',
        'EN': 'Instruction'
    },
    //////////////////////////////////////////////// public
    '订单管理': {
        'ZH': '订单管理',
        'EN': 'Orders'
    },
    '我要买入': {
        'ZH': '我要买入',
        'EN': 'Buy'
    },
    '我要出售': {
        'ZH': '我要出售',
        'EN': 'Sell'
    },
    ////////////////////////////////////////////////币币交易订单
    '委托价': {
        'ZH': '委托价',
        'EN': 'Price'
    },
    '总额': {
        'ZH': '总额',
        'EN': 'Total'
    },
    '已成交量': {
        'ZH': '已成交量',
        'EN': 'Executed Qty'
    },
    '未成交量': {
        'ZH': '未成交量',
        'EN': 'Unexecuted Qty'
    },
    '已成交额': {
        'ZH': '已成交额',
        'EN': 'Executed amt'
    },

    ///////////////////////////////////////////// 用户中心
    '用户资料': {
        'ZH': '用户资料',
        'EN': 'Profile'
    },
    '基本信息': {
        'ZH': '基本信息',
        'EN': 'Basic Information'
    },
    '安全设置': {
        'ZH': '安全设置',
        'EN': 'Security Setting'
    },
    '更换头像': {
        'ZH': '更换头像',
        'EN': 'Change'
    },
    '身份验证': {
        'ZH': '身份验证',
        'EN': 'ID Verification'
    },
    '电子邮件': {
        'ZH': '电子邮件',
        'EN': 'Email address'
    },
    '累计交易次数': {
        'ZH': '累计交易次数',
        'EN': 'Cumulative number of transactions'
    },
    '中国大陆身份证': {
        'ZH': '中国大陆身份证',
        'EN': 'ID card'
    },
    '适用于大多数中国大陆用户': {
        'ZH': '适用于大多数中国大陆用户',
        'EN': 'Applicable to users in mainland China'
    },
    '展开': {
        'ZH': '展开',
        'EN': 'Unfold'
    },
    '正面图片': {
        'ZH': '正面图片',
        'EN': 'The front of ID card'
    },
    '反面图片': {
        'ZH': '反面图片',
        'EN': 'The back of ID card'
    },
    '姓名': {
        'ZH': '姓名',
        'EN': 'Name'
    },
    '请输入姓名': {
        'ZH': '请输入姓名',
        'EN': 'Please enter your name'
    },
    '身份证号': {
        'ZH': '身份证号',
        'EN': 'ID card number'
    },
    '请输入身份证号': {
        'ZH': '请输入身份证号',
        'EN': 'Please enter your ID card number'
    },
    '确定': {
        'ZH': '确定',
        'EN': 'Submit'
    },
    '上传图片注意事项': {
        'ZH': '上传图片注意事项',
        'EN': 'Notes'
    },
    '您上传的身份证图片是': {
        'ZH': '您上传的身份证图片是',
        'EN': 'The photo of ID card you uploaded is'
    },
    '真实': {
        'ZH': '真实',
        'EN': 'real'
    },
    '有效的': {
        'ZH': '有效的',
        'EN': 'effective'
    },
    '您的身份证图片的信息': {
        'ZH': '您的身份证图片的信息',
        'EN': 'The information of your photo of ID card'
    },
    '清晰': {
        'ZH': '清晰',
        'EN': 'Clearly'
    },
    '可见': {
        'ZH': '可见',
        'EN': 'visible'
    },
    '您上传的照片没有反光或是折痕情况': {
        'ZH': '您上传的照片没有反光或是折痕情况',
        'EN': 'The photo you uploaded is not reflective or creased'
    },
    '您上传的图片为': {
        'ZH': '您上传的图片为',
        'EN': 'The photo you uploaded is'
    },
    '彩色': {
        'ZH': '彩色',
        'EN': 'color'
    },
    '图像': {
        'ZH': '图像',
        'EN': 'image'
    },
    '护照': {
        'ZH': '护照',
        'EN': 'Passport'
    },
    '适用于中国港澳台及其他外国用户': {
        'ZH': '适用于中国港澳台及其他外国用户',
        'EN': 'Applicable to Hong Kong, Macao, Taiwan and other foreign users'
    },
    '证件号': {
        'ZH': '证件号',
        'EN': 'Passport number'
    },
    '请输入证件号': {
        'ZH': '请输入证件号',
        'EN': 'Please enter your passport number'
    },
    '驾照': {
        'ZH': '驾照',
        'EN': 'Driving license'
    },
    '适用于非中国大陆用户': {
        'ZH': '适用于非中国大陆用户',
        'EN': 'Applicable to non-chinese mainland users'
    },
    '您上传的护照图片是': {
        'ZH': '您上传的护照图片是',
        'EN': 'The passport photo you uploaded is'
    },
    '您的护照图片的信息': {
        'ZH': '您的护照图片的信息',
        'EN': 'The information from your passport photo is'
    },
    '您上传的驾照图片是': {
        'ZH': '您上传的驾照图片是',
        'EN': 'The picture of driving license you uploaded is'
    },
    '您的驾照图片的信息': {
        'ZH': '您的驾照图片的信息',
        'EN': 'Information about your driving license photo'
    },
    '为确保交易安全...': {
        'ZH': '为确保交易安全，保障您的合法权益，请您完成身份验证！我们提供安全高效实时联网的官方身份认证服务，具有更高级别的安全保证。身份验证一旦成功，不予修改和解除认证。建议您慎重选择，真实填写。',
        'EN': 'To secure the transaction and protect your legal rights and interests, please complete your authentication！'
    },
    // ***** 安全设置
    '提现、修改安全设置时输入': {
        'ZH': '提现、修改安全设置时输入',
        'EN': 'Enter when withdraw and modify security settings'
    },
    '修改': {
        'ZH': '修改',
        'EN': 'Edit'
    },
    '立即设置': {
        'ZH': '立即设置',
        'EN': 'Bind'
    },
    '绑定邮箱': {
        'ZH': '绑定邮箱',
        'EN': 'Bind email address'
    },
    '已绑定': {
        'ZH': '已绑定',
        'EN': 'Bind'
    },
    '绑定手机号': {
        'ZH': '绑定手机号',
        'EN': 'Bind cellphone number'
    },
    '登录密码': {
        'ZH': '登录密码',
        'EN': 'Login password'
    },
    '用户登录账户时输入': {
        'ZH': '用户登录账户时输入',
        'EN': 'Enter when users log in to the account'
    },
    '谷歌验证': {
        'ZH': '谷歌验证',
        'EN': 'Google authentication'
    },
    '谷歌验证码': {
        'ZH': '谷歌验证码',
        'EN': 'Google verification code'
    },
    '绑定后,登录、提现时需要谷歌密码二次验证': {
        'ZH': '绑定后,登录、提现时需要谷歌密码二次验证',
        'EN': 'After binding, login and withdrawal need to be re-validated by Google password.'
    },
    '当前位置': {
        'ZH': '当前位置',
        'EN': 'The current position'
    },
    '请输入手机号': {
        'ZH': '请输入手机号',
        'EN': 'Please enter your cellphone number'
    },
    '请输入交易密码': {
        'ZH': '请输入交易密码',
        'EN': 'Please enter trading password.'
    },
    '验证码': {
        'ZH': '验证码',
        'EN': 'Please enter the code'
    },
    '获取验证码': {
        'ZH': '获取验证码',
        'EN': 'Get code'
    },
    '邮箱': {
        'ZH': '邮箱',
        'EN': 'mailbox'
    },
    '设置邮箱': {
        'ZH': '设置邮箱',
        'EN': 'Set up your email'
    },
    '请输入邮箱': {
        'ZH': '请输入邮箱',
        'EN': 'Please enter email address'
    },
    '邮箱验证码': {
        'ZH': '邮箱验证码',
        'EN': 'Mailbox verification code'
    },
    '获取邮箱验证码': {
        'ZH': '获取邮箱验证码',
        'EN': 'Get the mailbox verification code'
    },
    '手机号': {
        'ZH': '手机号',
        'EN': 'cellphone number'
    },
    '手机验证码': {
        'ZH': '手机验证码',
        'EN': 'cellphone verification code'
    },
    '获取手机验证码': {
        'ZH': '获取手机验证码',
        'EN': 'Get cellphone verification code'
    },
    '请输入原密码': {
        'ZH': '请输入原密码',
        'EN': 'Please enter the original password'
    },
    '请输入6-16位的新密码': {
        'ZH': '请输入6-16位的新密码',
        'EN': 'Please enter a new 6-16-bit password'
    },
    '请再次输入新密码': {
        'ZH': '请再次输入新密码',
        'EN': 'Please enter the new password again'
    },
    '评价人': {
        'ZH': '评价人',
        'EN': 'appraiser'
    },
    '是否好评': {
        'ZH': '是否好评',
        'EN': 'Whether the high praise'
    },
    '评价内容': {
        'ZH': '评价内容',
        'EN': 'Comment content'
    },
    '评价时间': {
        'ZH': '评价时间',
        'EN': 'Comment time'
    },
    '暂无评论': {
        'ZH': '暂无评论',
        'EN': 'No comments'
    },
    ////////////////////////////////// 登录
    '请输入账号': {
        'ZH': '请输入账号',
        'EN': 'Please enter the account number'
    },
    '请输入': {
        'ZH': '请输入',
        'EN': 'Please enter a'
    },
    '位的登录密码': {
        'ZH': '位的登录密码',
        'EN': 'bit login password'
    },
    '请输入6-16位的登录密码': {
        'ZH': '请输入6-16位的登录密码',
        'EN': 'Please enter a 6-16 bit login password'
    },
    '忘记密码': {
        'ZH': '忘记密码',
        'EN': 'Forgot password'
    },
    //////////////////////////////// 注册
    '手机注册': {
        'ZH': '手机注册',
        'EN': 'cellphone registered'
    },
    '邮箱注册': {
        'ZH': '邮箱注册',
        'EN': 'Email registered'
    },
    '请输入用户名': {
        'ZH': '请输入用户名',
        'EN': 'Please enter a username'
    },
    '请输入验证码': {
        'ZH': '请输入验证码',
        'EN': 'Please enter the verification code'
    },
    '我已阅读并接受': {
        'ZH': '我已阅读并接受',
        'EN': 'I have read and accepted'
    },
    '产品服务条款': {
        'ZH': 'tychely产品服务条款',
        'EN': 'Product terms and conditions'
    },
    '注册协议': {
        'ZH': '注册协议',
        'EN': 'The registration agreement'
    },
    ///////////////////////////////////// 找回密码
    '请输入手机号或邮箱': {
        'ZH': '请输入手机号或邮箱',
        'EN': 'Please enter your cellphone number or email address'
    },
    '请确认密码': {
        'ZH': '请确认密码',
        'EN': 'Please confirm the password.'
    },
    ////////////////////////////////////
    '位的新密码': {
        'ZH': '位的新密码',
        'EN': 'bit password'
    },
    '挂单': {
        'ZH': '挂单',
        'EN': 'entry Orders'
    },
    '吃单': {
        'ZH': '吃单',
        'EN': 'transact Orders'
	},
	'买': {
        'ZH': '买',
        'EN': 'Buy'
    },
    '卖': {
        'ZH': '卖',
        'EN': 'Sell'
    },
    '市价': {
        'ZH': '市价',
        'EN': 'Market price'
	},
	'价格不能小于等于': {
        'ZH': '价格不能小于等于',
        'EN': 'The price cannot be less than or equal to'
    },
    '订单提交成功': {
        'ZH': '订单提交成功',
        'EN': 'Order submitted successfully'
    },
    '订单提交失败': {
        'ZH': '订单提交失败',
        'EN': 'Order submission failed'
	},
    '小数点后最大不得大于八位': {
        'ZH': '小数点后最大不得大于八位',
        'EN': 'The maximum number of decimal places should not be more than eight'
    },
    '以市场上最优价格买入': {
        'ZH': '以市场上最优价格买入',
        'EN': 'Buy at the best price in the market'
	},
	'以市场最优价格卖出': {
        'ZH': '以市场最优价格卖出',
        'EN': "Sell at the market's best price"
    },
    '累计': {
        'ZH': '累计',
        'EN': 'The cumulative'
    },
    '分时': {
        'ZH': '分时',
        'EN': 'Time-sharing'
	},
	'详情': {
        'ZH': '详情',
        'EN': 'Details'
    },
    '成交额': {
        'ZH': '成交额',
        'EN': 'Turnover'
    },
    '订单將在拖管中保持至': {
        'ZH': '订单將在拖管中保持至',
        'EN': 'Orders will be kept in'
    },
    '逾期未支付交易將自动取消': {
        'ZH': '逾期未支付交易將自动取消',
        'EN': 'Overdue transactions will be cancelled automatically'
    },
    '卖家信息': {
        'ZH': '卖家信息',
        'EN': 'The seller information'
    },
    '买家信息': {
        'ZH': '买家信息',
        'EN': 'The buyer information'
	},
	'广告详情': {
        'ZH': '广告详情',
        'EN': 'Advertisement Details'
    },
    '购买订单': {
        'ZH': '购买订单',
        'EN': 'Purchase order'
    },
    '出售订单': {
        'ZH': '出售订单',
        'EN': 'Selling orders'
	},
	'已验证': {
        'ZH': '已验证',
        'EN': 'Verified'
    },
    '未验证': {
        'ZH': '未验证',
        'EN': 'Unverified'
	},
	'建立连接成功': {
        'ZH': '建立连接成功',
        'EN': 'Connect successfully'
    },
    '连接已断开，无法收到新消息，请检查下你的网络是否正常': {
        'ZH': '连接已断开，无法收到新消息，请检查下你的网络是否正常',
        'EN': 'The connection has been disconnected and no new messages can be received. Please check if your network is normal'
    },
    '连接状态恢复正常': {
        'ZH': '连接状态恢复正常',
        'EN': 'Connection status returned to normal'
	},
	'未知连接状态': {
        'ZH': '未知连接状态',
        'EN': 'Unknown connection status'
    },
    '该群组已开启全局禁言': {
        'ZH': '该群组已开启全局禁言',
        'EN': 'The group has been banned to chat.'
	},
	'该群还没有历史消息': {
        'ZH': '该群还没有历史消息',
        'EN': 'The group has no history yet'
    },
    '该群没有历史消息了': {
        'ZH': '该群没有历史消息了',
        'EN': 'The group has no history'
    },
    '该群没有历史消息可拉取了': {
        'ZH': '该群没有历史消息可拉取了',
        'EN': 'The group has no historical information to draw'
	},
	'获取下一次拉取的群消息seq为空': {
        'ZH': '获取下一次拉取的群消息seq为空',
        'EN': 'Gets the next pull of the group message seq is empty'
    },
    '消息长度超出限制(最多3个汉字)': {
        'ZH': '消息长度超出限制(最多3个汉字)',
        'EN': 'Message length exceeded limit (up to 3 characters)'
	},
	'消息发送失败，请重新发送': {
        'ZH': '消息发送失败，请重新发送',
        'EN': 'Message sending failed. Please resend'
    },
    '系统消息': {
        'ZH': '系统消息',
        'EN': 'System message'
    },
    '未知消息元素类型': {
        'ZH': '未知消息元素类型',
        'EN': 'Unknown message element type'
	},// *****//
	'您的浏览器不支持': {
        'ZH': '您的浏览器不支持',
        'EN': 'Your browser does not support it'
	},
	'您还没有好友，暂不能聊天': {
        'ZH': '您还没有好友，暂不能聊天',
        'EN': "You have no friends, so you can't talk"
	},
	'您选中的文件不是图片，请重新选择': {
        'ZH': '您选中的文件不是图片，请重新选择',
        'EN': 'The file you selected is not an image, please reselect it'
	},
	'您选择的图片大小超过限制(最大为30M)，请重新选择': {
        'ZH': '您选择的图片大小超过限制(最大为30M)，请重新选择',
        'EN': 'The image size you selected exceeds the limit (maximum 30M), please reselect it'
	},
	'确认取消交易？': {
        'ZH': '确认取消交易？',
        'EN': 'Confirm cancellation?'
    },
	'操作成功': {
        'ZH': '操作成功',
        'EN': 'Action successfully'
	},
	'确认标记打款？': {
        'ZH': '确认标记打款？',
        'EN': 'Confirm mark payment?'
	},
	'确认解冻货币？': {
        'ZH': '确认解冻货币？',
        'EN': 'Confirm unfrozen currency？'
	},
	'操作成功, 其中含有关键字，需平台进行审核': {
        'ZH': '操作成功, 其中含有关键字，需平台进行审核',
        'EN': 'Successful action, which contains keywords, needs  audit.'
    },
	'购买': {
        'ZH': '购买',
        'EN': 'Buy '
    },
	'出售': {
        'ZH': '出售',
        'EN': 'Sell '
	},
	'取消订单': {
        'ZH': '取消订单',
        'EN': 'Cancel'
	},
	'交谈中': {
        'ZH': '交谈中',
        'EN': 'In conversation'
	},
	'最高价格': {
        'ZH': '最高价格',
        'EN': 'The highest price'
	},
	'请输请入购买币的总量': {
        'ZH': '请输请入购买币的总量',
        'EN': 'Please enter the total amount of the purchased currency'
	},
	'最低价格': {
        'ZH': '最低价格',
        'EN': 'Lowest price'
    },
    '最低价（选填）': {
        'ZH': '最低价（选填）',
        'EN': 'Lowest price (Selection)'
    },
	'广告最低可成交的价格': {
        'ZH': '广告最低可成交的价格',
        'EN': 'The lowest possible price for advertising'
	},
	'出售总量': {
        'ZH': '出售总量',
        'EN': 'Total available amount'
	},
	'请输入售卖币的总量': {
        'ZH': '请输入售卖币的总量',
        'EN': 'Please enter total available amount'
	},
	'隐藏高级设置': {
        'ZH': '隐藏高级设置',
        'EN': 'Hide advanced Settings'
    },
	'请输入数字': {
        'ZH': '请输入数字',
        'EN': 'Please enter the number'
    },
	'确认下架此广告？': {
        'ZH': '确认下架此广告？',
        'EN': 'Confirm unrelease advertisement?'
	},
	'下单成功': {
        'ZH': '下单成功',
        'EN': 'Success'
	},
    '购买成功': {
        'ZH': '购买成功',
        'EN': 'Success'
    },
	'请先设置交易密码': {
        'ZH': '请先设置交易密码',
        'EN': 'Please set fund password first'
	},
	'编辑': {
        'ZH': '编辑',
        'EN': 'Edit'
	},
	'信任': {
        'ZH': '信任',
        'EN': 'Trusted'
	},
	'出售成功': {
        'ZH': '出售成功',
        'EN': 'Success'
    },
	'密码重置成功': {
        'ZH': '密码重置成功',
        'EN': 'Password reset successfully'
	},
	'认证审核不通过，请重新认证！': {
        'ZH': '认证审核不通过，请重新认证！',
        'EN': 'Certification is not approved, please recertify!'
	},
	'收起': {
        'ZH': '收起',
        'EN': 'Pack up'
	},
	'认证请求发起成功': {
        'ZH': '认证请求发起成功',
        'EN': 'The authentication request is successful'
    },
	'登录成功': {
        'ZH': '登录成功',
        'EN': 'Login success'
    },
	'查看': {
        'ZH': '查看',
        'EN': 'view'
	},
	'查看详情': {
        'ZH': '查看详情',
        'EN': 'Details'
	},
	'注册成功': {
        'ZH': '注册成功',
        'EN': 'Registered success'
	},
	'重新发送': {
        'ZH': '重新发送',
        'EN': 'Resend'
	},
	'设置成功': {
        'ZH': '设置成功',
        'EN': 'Success'
	},
	'开启成功': {
        'ZH': '开启成功',
        'EN': 'Success'
    },
	'关闭成功': {
        'ZH': '关闭成功',
        'EN': 'Success'
	},
	'修改成功': {
        'ZH': '修改成功',
        'EN': 'Success'
	},
	'未绑定': {
        'ZH': '未绑定',
        'EN': 'Unbind'
	},
	'请选择图片': {
        'ZH': '请选择图片',
        'EN': 'Please select picture'
    },
	'已信任': {
        'ZH': '已信任',
        'EN': 'Trusted'
    },
	'已拉黑': {
        'ZH': '已拉黑',
        'EN': 'Blacklisted'
	},
	'屏蔽': {
        'ZH': '屏蔽',
        'EN': 'Block'
	},
	'邮箱已验证': {
        'ZH': '邮箱已验证',
        'EN': 'Email verified'
	},
	'邮箱未验证': {
        'ZH': '邮箱未验证',
        'EN': 'Email unverified'
	},
	'手机已验证': {
        'ZH': '手机已验证',
        'EN': 'Cellphone verified'
	},
	'手机未验证': {
        'ZH': '手机未验证',
        'EN': 'Cellphone unverified'
    },
	'身份已验证': {
        'ZH': '身份已验证',
        'EN': 'Authenticated'
	},
	'身份未验证': {
        'ZH': '身份未验证',
        'EN': 'Unauthenticated'
	},
	'已取消信任': {
        'ZH': '已取消信任',
        'EN': 'Untrusted'
	},
	'已取消拉黑': {
        'ZH': '已取消拉黑',
        'EN': 'Unblacklisted'
    },
	'是': {
        'ZH': '是',
        'EN': 'Yes'
    },
	'否': {
        'ZH': '否',
        'EN': 'No'
	},
	'待支付': {
        'ZH': '待支付',
        'EN': 'Pending to pay'
	},
	'待确认': {
        'ZH': '待确认',
        'EN': 'Pending to confirm'
	},
	'已完成': {
        'ZH': '已完成',
        'EN': 'Completed'
	},
	'已取消': {
        'ZH': '已取消',
        'EN': 'cancelled'
	},
	'平台已取消': {
        'ZH': '平台已取消',
        'EN': ' cancelled'
    },
	'单价': {
        'ZH': '单价',
        'EN': 'Unit price'
	},
	'金额': {
        'ZH': '金额',
        'EN': 'Amount'
	},
	'单笔限制': {
        'ZH': '单笔限制',
        'EN': 'Single limit'
	},
    '最低额度': {
        'ZH': '最低额度',
        'EN': 'Min'
    },
	'请输入购买金额': {
        'ZH': '请输入购买金额',
        'EN': 'Please enter the purchase amount'
    },
	'请输入自己的支付宝账号（以便确认）': {
        'ZH': '请输入自己的支付宝账号（以便确认）',
        'EN': 'Please enter your alipay account (for confirmation)'
    },
	'付款备注里不得出现 BTC/ETH、数字货币、区块链等字眼。': {
        'ZH': '付款备注里不得出现 BTC/ETH、数字货币、区块链等字眼。',
        'EN': 'No BTC/ETH, digital currency, blockchain, etc.'
	},
	'请输入卖出金额': {
        'ZH': '请输入卖出金额',
        'EN': 'Please enter the selling amount'
	},
    '请输入出售金额': {
        'ZH': '请输入出售金额',
        'EN': 'Please enter the selling amount'
    },
    '请输入出售数量': {
        'ZH': '请输入出售金额数量',
        'EN': 'Please enter the selling quantity'
    },
	'请输入账号或卡号': {
        'ZH': '请输入账号或卡号',
        'EN': 'Please enter your account number.'
	},
	'提币': {
        'ZH': '提币',
        'EN': 'Withdraw'
	},
	'去交易': {
        'ZH': '去交易',
        'EN': 'To trade'
    },
	'接收地址': {
        'ZH': '接收地址',
        'EN': 'Receiving address'
	},
	'温馨提示': {
        'ZH': '温馨提示',
        'EN': 'Warning'
	},
	'地址只能充值': {
        'ZH': '地址只能充值',
        'EN': 'Addresses can only be charged'
	},
	// '任何充入': {
    //     'ZH': '任何充入',
    //     'EN': 'transact Orders'
    // },
	// '地址的非资产将不可找回。': {
    //     'ZH': '地址的非资产将不可找回。',
    //     'EN': 'transact Orders'
    // },
	'在平台内相互转账是实时到账且免费的。': {
        'ZH': '在平台内相互转账是实时到账且免费的。',
        'EN': 'Mutual transfers within the platform are real-time and free of charge.'
	},
	'提现地址': {
        'ZH': '提现地址',
        'EN': 'Address'
	},
	'请输入提现地址': {
        'ZH': '请输入提现地址',
        'EN': 'Please enter the withdrawal address'
	},
	'提现数量': {
        'ZH': '提现数量',
        'EN': 'Amount'
    },
    '请输入提现数量': {
        'ZH': '请输入提现数量',
        'EN': 'Please enter the withdrawal amount'
	},
	'忘记密码？': {
        'ZH': '忘记密码？',
        'EN': 'Forget password?'
    },
    '谷歌验证码': {
        'ZH': '谷歌验证码',
        'EN': 'Google verification code'
	},
	'请输入谷歌验证码': {
        'ZH': '请输入谷歌验证码',
        'EN': 'Please enter the Google verification code'
    },
	'备注': {
        'ZH': '备注',
        'EN': 'Remark'
	},
	'请输入提现备注': {
        'ZH': '请输入提现备注',
        'EN': 'Please enter the notes'
	},
	'确定提现': {
        'ZH': '确定提现',
        'EN': 'Confirm'
	},
	'暂无地址': {
        'ZH': '暂无地址',
        'EN': 'No address'
    },
	'标签': {
        'ZH': '标签',
        'EN': 'Label'
    },
	'请输入卖出数量': {
        'ZH': '请输入卖出数量',
        'EN': 'Please enter the selling quantity'
	},
	'请输入购买数量': {
        'ZH': '请输入购买数量',
        'EN': 'Please enter the purchase quantity'
	},
	'输入金额不在限额之内，请重新输入！': {
        'ZH': '输入金额不在限额之内，请重新输入！',
        'EN': 'Input amount is not within the limit, please input again!'
	},
	'加载失败': {
        'ZH': '加载失败',
        'EN': 'Loading fail'
	},
	'加载中': {
        'ZH': '加载中',
        'EN': 'Loading'
	},
	'图片文件': {
        'ZH': '图片文件',
        'EN': 'Image files'
    },
	'不能为空': {
        'ZH': '不能为空',
        'EN': "Can't be empty"
	},
	'请输入正确格式的电子邮件': {
        'ZH': '请输入正确格式的电子邮件',
        'EN': 'Please enter the email in the correct format'
	},
	'请输入合法的网址': {
        'ZH': '请输入合法的网址',
        'EN': 'Please enter a valid url'
	},
	'请输入合法的日期': {
        'ZH': '请输入合法的日期',
        'EN': 'Please enter a valid date'
    },
	'请输入合法的数字': {
        'ZH': '请输入合法的数字',
        'EN': 'Please enter a valid number'
    },
	'只能输入整数': {
        'ZH': '只能输入整数',
        'EN': 'You can only enter integers'
	},
	'请输入合法的信用卡号': {
        'ZH': '请输入合法的信用卡号',
        'EN': 'Please enter a valid credit card number'
	},
	'两次密码不同': {
        'ZH': '两次密码不同',
        'EN': 'Two different passwords'
	},
	'请输入拥有合法后缀名的字符串': {
        'ZH': '请输入拥有合法后缀名的字符串',
        'EN': 'Enter a string with a valid suffix'
	},
	'格式错误': {
        'ZH': '格式错误',
        'EN': 'Invalid format'
	},
	'请输入合法字符': {
        'ZH': '请输入合法字符',
        'EN': 'Please enter legal characters'
    },
	'只能输入中文': {
        'ZH': '只能输入中文',
        'EN': 'Chinese only'
	},
	'请输入英文名称': {
        'ZH': '请输入英文名称',
        'EN': 'Please input English name'
	},
	'不能和原始密码相同': {
        'ZH': '不能和原始密码相同',
        'EN': 'Cannot be the same as the original password'
	},
	'不能小于0 或空': {
        'ZH': '不能小于0 或空',
        'EN': "It can't be less than 0 or null"
    },
	'请输入大于0的数字': {
        'ZH': '请输入大于0的数字',
        'EN': 'Please enter a number greater than 0'
    },
	'请输入正整数': {
        'ZH': '请输入正整数',
        'EN': 'Please enter a positive integer'
	},
	'邮箱格式错误': {
        'ZH': '邮箱格式错误',
        'EN': 'Email format error'
	},
	'手机格式错误': {
        'ZH': '手机格式错误',
        'EN': 'Cell phone format error'
	},
	'邮箱或手机格式错误': {
        'ZH': '邮箱或手机格式错误',
        'EN': 'Email or cellphone format error'
	},
	'电话或手机格式错误': {
        'ZH': '电话或手机格式错误',
        'EN': 'Cellphone format error'
	},
	'验证码格式错误': {
        'ZH': '验证码格式错误',
        'EN': 'Validation code format error'
    },
	'交易密码长度为6位且为数字': {
        'ZH': '交易密码长度为6位且为数字',
        'EN': 'The password length is 6 digits'
	},
	'字母数字组合6-16位': {
        'ZH': '字母数字组合6-16位',
        'EN': 'Alphanumeric combination 6-16 bits'
	},
	'登录超时，请重新登录': {
        'ZH': '登录超时，请重新登录',
        'EN': 'Login timeout, please login again'
    },
    '跳转': {
        'ZH': '跳转',
        'EN': 'Skip'
    },
    '共': {
        'ZH': '共',
        'EN': 'All'
    },
    '页': {
        'ZH': '页',
        'EN': ''
    },
    '区块链技术应用实验平台': {
        'ZH': '区块链技术应用实验平台',
        'EN': ' blockchain technology application experimental platform'
    },
    '已取消支付': {
        'ZH': '已取消支付',
        'EN': 'Cancelled payment'
    },
    '取消支付': {
        'ZH': '取消支付',
        'EN': 'Cancel the payment'
    },
    '出售详情': {
        'ZH': '出售详情',
        'EN': 'Selling details'
    },
    '您有未读消息': {
        'ZH': '您有未读消息',
        'EN': 'New message'
    },
    '您有其他未读消息': {
        'ZH': '您有其他未读消息',
        'EN': 'New message'
    },
    '个人主页': {
        'ZH': '个人主页',
        'EN': 'Personal homepage'
    },
    '人': {
        'ZH': '人',
        'EN': 'People'
    },
    '未读': {
        'ZH': '未读',
        'EN': 'Unread'
    },
    'otc商家': {
        'ZH': 'otc商家',
        'EN': 'Otc business'
    },
    // 腾讯云
    '系统消息：交易已下单，等待买家标记打款': {
        'ZH': '系统消息：交易已下单，等待买家标记打款',
        'EN': 'System Message: The transaction has placed an order waiting for the buyer to mark the payment'
    },
    '系统消息：订单支付超时，系统自动取消': {
        'ZH': '系统消息：订单支付超时，系统自动取消',
        'EN': 'System Message: Order payment timeout, system cancel automatically'
    },
    '系统消息：订单已取消': {
        'ZH': '系统消息：订单已取消',
        'EN': 'System Message: Order has cancelled'
    },
    '系统消息：买家已标记打款': {
        'ZH': '系统消息：买家已标记打款',
        'EN': 'System Message: Buyer has marked the payment'
    },
    '系统消息：卖家已释放': {
        'ZH': '系统消息：卖家已释放',
        'EN': 'System Message: Seller has released'
    },
    '系统消息：订单已申请仲裁': {
        'ZH': '系统消息：订单已申请仲裁',
        'EN': 'System Message: Order has applied for arbitration'
    },
    '系统消息：仲裁申请已处理完成': {
        'ZH': '系统消息：仲裁申请已处理完成',
        'EN': 'System Message: The arbitration application has been processed'
    },
    '系统消息：买家申请仲裁不通过，买家实际未付款，订单取消': {
        'ZH': '系统消息：买家申请仲裁不通过，买家实际未付款，订单取消',
        'EN': 'System Message: Buyer to apply for arbitration does not pass, the buyer is not actually paid, order cancelled'
    },
    '系统消息：买家申请仲裁通过，买家实际已付款，自动解冻': {
        'ZH': '系统消息：买家申请仲裁通过，买家实际已付款，自动解冻',
        'EN': 'System message: Buyer apply for arbitration through the buyer has actually paid, automatic thawing'
    },
    '系统消息：买家已评价': {
        'ZH': '系统消息：买家已评价',
        'EN': 'System Message: Buyer has evaluated'
    },
    '系统消息：卖家已评价': {
        'ZH': '系统消息：卖家已评价',
        'EN': 'System Message: Seller has evaluated'
    },
    // 流水
    '提交卖出[ETH]委托单': {
        'ZH': '提交卖出[ETH]委托单',
        'EN': 'Commit sell ETH order'
    },
    '提交购买[ETH]委托单': {
        'ZH': '提交购买[ETH]委托单',
        'EN': 'Commit buy ETH order'
    },
    '提交卖出[BTC]委托单': {
        'ZH': '提交卖出[BTC]委托单',
        'EN': 'Selling orders [BTC]'
    },
    '提交购买[BTC]委托单': {
        'ZH': '提交购买[BTC]委托单',
        'EN': 'Buy BTC order'
    },
    '充币-来自地址': {
        'ZH': '充币-来自地址',
        'EN': 'Charge-from address '
    },
    '充币-来自交易': {
        'ZH': '充币-来自交易',
        'EN': 'Charge-from transactional '
    },
    '充币-交易id': {
        'ZH': '充币-交易id',
        'EN': 'Charge-from id'
    },
    '充币-外部地址': {
        'ZH': '充币-外部地址',
        'EN': 'Charge-from external address'
    }
}


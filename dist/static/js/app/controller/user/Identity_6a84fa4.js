...
define('js/app/controller/user/Identity', ['js/app/controller/base', 'js/app/util/ajax', 'js/app/interface/GeneralCtr', 'js/app/module/qiniu/index', 'js/app/module/validate/validate', 'js/app/interface/UserCtr', 'js/app/controller/Top', 'js/app/controller/foo'], function (base, Ajax, GeneralCtr, QiniuUpdata, validate, UserCtr, Top, Foo) {
    ...
    function init() {
        ...
        setHtml();
        $.when(getUser(), getQiniuToken(sf_photoFile));
    ...
    function getUser() {
        return UserCtr.getUser().then(function (data) {
            var idAuthStatus = '';
            var userIdAuthInfo = data.userIdAuthInfo;
    ...
    function getQiniuToken(sf_photoFile) {
        return GeneralCtr.getQiniuToken().then(function (data) {
            var token = data.uploadToken;
            base.showLoadingSpin();
            sf_photoFile.forEach(function (item) {
    ...
    function addListener() {
        ...
        $('#sf_subBtn').off('click').click(function () {
            ...
            if (_registerForm.valid()) {
                userSFVerify(userConfig).then(function (data) {
                    loadFn();
                });
            }
        });

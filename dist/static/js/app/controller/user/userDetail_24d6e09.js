...
define('js/app/controller/user/userDetail', ['js/app/controller/base', 'js/lib/pagination/jquery.pagination', 'js/app/module/validate/validate', 'js/app/module/smsCaptcha/smsCaptcha', 'js/app/interface/AccountCtr', 'js/app/interface/GeneralCtr', 'js/app/interface/UserCtr', 'js/app/interface/TradeCtr', 'js/app/controller/Top', 'js/app/controller/foo'], function (base, pagination, Validate, smsCaptcha, AccountCtr, GeneralCtr, UserCtr, TradeCtr, Top, Foo) {
    ...
    function init() {
        ...
        // getUserRelation() // 测试
        // 查询币种和付款方式
        $.when(GeneralCtr.getDictList({ "parentKey": "coin" }), GeneralCtr.getDictList({ "parentKey": "pay_type" }), getUserRelation(), // 正式
        getUserDetail()).then(function (data1, data2) {
    ...
    function getUserRelation() {
        return UserCtr.getUserRelation(currency, userId).then(function (data) {
    ...
    function getUserDetail() {
        return UserCtr.getUser(true, userId).then(function (data) {
            var photoHtml = "";
            // 头像
            if (data.photo) {
            ...
            var jiaoYiCount = data.userStatistics ? data.userStatistics.jiaoYiCount : '-';
            var beiXinRenCount = data.userStatistics ? data.userStatistics.beiXinRenCount : '-';
            var beiHaoPingCount = data.userStatistics ? data.userStatistics.beiHaoPingCount : '-';
            var beiPingJiaCount = data.userStatistics ? data.userStatistics.beiPingJiaCount : '-';
    ...
    function getPageAdvertise() {
        TradeCtr.getPageAdvertiseUser(config, true).then(function (data) {
            var lists = data.list;
            if (data.list.length) {
                var html = "";
    ...
    function addListener() {
        ...
        $('.k-userbtn .trust').click(function () {
            ...
            if (_this.attr("data-isTrust") == '1') {
                ...
                UserCtr.removeUserRelation(relationConfig, true).then(function (data) {
                ...
                UserCtr.addUserRelation(relationConfig, true).then(function (data) {

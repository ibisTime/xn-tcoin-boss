...
define('js/app/controller/user/Trust', ['js/app/controller/base', 'js/lib/pagination/jquery.pagination', 'js/app/interface/UserCtr', 'js/app/controller/Top', 'js/app/controller/foo', 'js/app/controller/public/DealLeft'], function (base, pagination, UserCtr, Top, Foo, DealLeft) {
    var langType = localStorage.getItem('langType') || 'ZH';
    var type = base.getUrlParam("trustType") || '1'; // 0: 你屏蔽的人，1:你信任的人，2:信任你的人
    var gohref = base.getUrlParam('go') || 'm_xr'; // m_xr: 你信任的人, xr_m: 信任你的人, m_pb:你屏蔽的人
    var config = {
        start: 1,
        limit: 10
    };
    ...
    function getPageTrust(params, to) {
        return UserCtr.getPageTrust(params, to).then(function (data) {
            var lists = data.list;
            if (lists.length > 0) {
                var html = "";

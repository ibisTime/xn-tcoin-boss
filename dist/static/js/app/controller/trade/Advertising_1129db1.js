...
define('js/app/controller/trade/Advertising', ['js/app/controller/base', 'js/app/module/validate/validate', 'js/app/interface/GeneralCtr', 'js/app/interface/UserCtr', 'js/app/interface/TradeCtr', 'js/app/interface/AccountCtr', 'js/app/controller/Top', 'js/app/controller/foo'], function (base, Validate, GeneralCtr, UserCtr, TradeCtr, AccountCtr, Top, Foo) {
    ...
    function init() {
        ...
        $.when(GeneralCtr.getSysConfig("trade_remind"), GeneralCtr.getDictList({ "parentKey": "trade_time_out" }), getAdvertisePrice(), GeneralCtr.getDictList({ "parentKey": "pay_type" }), getExplain('sell'), getAccount(coin.toUpperCase())).then(function (data1, data2, data3, data4) {
            //说明
            $("#tradeWarn").html(data1.cvalue.replace(/\n/g, '<br>'));

            //付款时限
            var html = '';
            data2.reverse().forEach(function (item) {
    ...
    function getAdvertiseDetail() {
        return TradeCtr.getAdvertiseDetail(code).then(function (data) {
            ...
            var premiumRate = (Math.floor(data.premiumRate * 1000) / 100).toFixed(2);
    ...
    function getExplain(type) {
        ...
        return GeneralCtr.getSysConfigType(param, true).then(function (data) {
    ...
    function addListener() {
        ...
        $('.yj-num').keyup(function () {
            //(parWidth * data.premiumRate) / 100;
            var leftValue = parseFloat($(".yj-num").val());
            ...
            var parWidth = $('.num-huadtiao').width();
            jdLeft = parWidth * leftValue / 100;
            isKup = false;
            var jdValue = (mid * (1 + leftValue / 100)).toFixed(2);
            var ccWidth = 50 / $('.num-huadtiao').width();
            $("#price").val(jdValue);
            $('.num-go').css({
        ...
        function doSubmit(publishType) {
            ...
            base.showLoadingSpin();
            return TradeCtr.submitAdvertise(params).then(function () {
        ...
        $("#doDownBtn").on("click", function () {
            ...
            base.confirm(base.getText('确认下架此广告？', langType), base.getText('取消', langType), base.getText('确认', langType)).then(function () {
                base.showLoadingSpin();
                TradeCtr.downAdvertise(code).then(function () {
                    base.hideLoadingSpin();
                    base.showMsg(base.getText('操作成功', langType));
                    setTimeout(function () {
                        history.go(-1);
        ...
        $('.advertise-step1-btn').on('click', function () {
            base.gohref('../trade/advertise-step2.html');
        });
        $('.advertise-step2-btn').on('click', function () {
        ...
        $('.num-go').mousedown(function (e) {
            ...
            i++;
            var parWidth = $('.num-huadtiao').width();
            var left = (goLeft / parWidth).toFixed(1) * 100;
            $('.go-box').mousemove(function (e) {
                var mlen = ((e.pageX - goX) / parWidth * 100).toFixed(2);
                if (mlen >= 50) {
                    mlen = 50;
                }
                if (mlen <= -50) {
        ...
        function advertiseData(mType) {
            var m_type = mType;
            var protectPrice = $('#protectPrice').val() || 0;
            var minTrade = $('#minTrade').val() || 0;
            var maxTrade = $('#maxTrade').val() || 0;
            getAdvertisePrice(coin, m_type).then(function (data) {
                mid = data.mid;
                var bblv = midBlv / mid;
                $("#price").val(mid);
                if (bblv < 0) {
        ...
        $('#tradeCurrency').change(function () {
            midBlv = $('#price').val() || 0;
            var receiveType = $(this).find("option:selected").val();
            switch (receiveType) {
                case 'CNY':
                    $('.m-type').text('CNY');
                    advertiseData('CNY');
        ...
        $('.advertise-step1-tabs .tab-item').on('click', function (e) {
            var target = e.target;
            // console.log($(target).attr('data-index'));
            $(target).addClass('active').siblings().removeClass('active');
        });
        $('.step3-vpn-checkbox .vpn-checkbox-item i').on('click', function (e) {
            var target = e.target;

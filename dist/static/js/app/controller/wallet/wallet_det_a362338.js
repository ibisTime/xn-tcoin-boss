...
define('js/app/controller/wallet/wallet_det', ['js/app/controller/base', 'js/lib/pagination/jquery.pagination', 'js/app/util/ajax', 'js/app/controller/Top', 'js/app/controller/foo'], function (base, pagination, Ajax, Top, Foo) {
    ...
    getBankData().then(function (data) {
    ...
    function getCTS() {
        getCTSData().then(function (data) {
            ...
            var realName = userCTSList.user.nickname;
            // 类型 买入
        ...
        $('.cz-btn button').off('click').click(function () {
            ...
            switch (selTxt) {
                case base.getText('我已完成付款', langType):
                    bjPlayfo(config).then(function () {
                        location.reload();
                    });
                    break;
                case base.getText('取消交易', langType):
                    qxOrder(config).then(function () {
                        location.reload();
                    });
                    break;
            }
